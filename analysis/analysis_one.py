...
# 计算歌曲重名
grouped_song_data = loved_song_data.groupby(
    "song_name"
)  # 从DataFrame转换成DataFrameGroupby
repeated_name_count = grouped_song_data.size().sort_values(
    ascending=False
)  # 从DataFramegroupby转换成Series
name_analysis = repeated_name_count.reset_index(
    name="record_count"
)  # 从Series转换成DataFrame
...
name_analysis["name_length_group"] = pandas.cut(
    name_analysis["name_length"],
...
length_analysis = pandas.DataFrame(
    {"total_name_count": total_name_count, "repeated_count": repeated_count}
)
length_analysis["repeated_ratio (%)"] = (
    length_analysis["repeated_count"] / length_analysis["total_name_count"] * 100
)
...
for length_name in length_analysis.index:
    repeated_count = length_analysis.loc[length_name, "repeated_count"]
    total_count = length_analysis.loc[length_name, "total_name_count"]
    ratio = length_analysis.loc[length_name, "repeated_ratio (%)"]
