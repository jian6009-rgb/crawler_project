...
artist_song_data = loved_song_data[["normalized_url", "singer_urls"]].explode(
    "singer_urls"
)
artist_song_data = artist_song_data.dropna(
    subset=["singer_urls"]
)  # 删除没有歌手网址的数据
artist_song_data["normalized_artist_url"] = artist_song_data["singer_urls"].apply(
    normalurl
)
...
artist_song_counts = (
    grouped_artist_data["normalized_url"].nunique().sort_values(ascending=False)
)

total_artist_count = len(artist_song_counts)
one_song_artist_count = (artist_song_counts == 1).sum()  # 只有1首歌曲的歌手数量
one_song_artist_ratio = (
    one_song_artist_count / total_artist_count
) * 100  # 只有1首歌曲的歌手占比
...
top10_percent_share = (
    artist_song_counts.head(top10_percent_count).sum() / total_song_count * 100
)
top20_percent_share = (
    artist_song_counts.head(top20_percent_count).sum() / total_song_count * 100
)
