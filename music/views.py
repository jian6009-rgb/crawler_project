...
def song_list(request):
    ...
    page_number = request.GET.get("pageNo") # 翻页工具
    ...
    return render(
        request, "music/song_list.html", {"songs": songs, "allpages": allpages}
    )


def song_detail(request, song_id):
    song = get_object_or_404(Song, id=song_id)
    if request.method == "POST": # 创建comment
