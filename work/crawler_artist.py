...
# 新增一些注释：由于Kugou一直有一些防止爬虫的东西，因此改用qianqian音乐，
# crawler_song.py是几乎复制crawler_kugo.py，可在那里找到原代码,而这个是复制crawler_song.py
USER_AGENT = (
...
# 写完这个代码后的小心思：其实这个代码能优化，如果把crawler_song.py和crawler_artist.py结合应该能让代码效率更高。
# 但是我当时心想一步一步来会更适合初学者，但结果让代码更复杂了。
# 原本可以先找遍历歌手，进歌手网站然后把歌手简介和歌全一次性爬出来。
# 但结果现在是先找歌曲写json，然后再json记录里找歌手，复杂了很多。而且我找歌曲2000首后发现其中有简介的歌手还没有100个，
# 结果就是最后又要找多几个榜单录歌，再从json记录里找歌手。所以json歌曲不止2000首。
...
def getartist_sources():  # 从json遍历歌曲，提取歌手url。并把信息制成字典：{歌手：歌曲s}
    ...
    for song in songs:
        singer_urls = song.get(
            "singer_urls", []
        )  # 其实也可写singer_urls = song["singer_urls"],但我没心思看json2000首歌是不是每个都有singer_urls。
...
def getartist_detail(
    ...
    artist_name = (
        page.locator(".info-box .info h1").inner_text().strip()
    )  # 从源代码html自己慢慢找的
    if not artist_name:
        print("没有名")
        return None
    ...
    if (
        ...
        or not artist_image_url.startswith("http")
    ):
        print("没有歌手图")
        return None
...
def playwrit():  # 先getartist_sources()获取歌手名单，
    ...
    if ARTIST_DATA_PATH.exists():  # 打开歌手json，先把已记录歌手名单提取以实现断点续爬
        # （用url和name分别查重进行双重保险，因为试过只用url查重的话，金莎一个歌手录了两次）
    ...
    with sync_playwright() as playwright:
        # 打开context,对歌手名单每个歌手进行getartist_detail(),然后saveartist()
        browser = playwright.chromium.launch(headless=True)
        context = browser.new_context(user_agent=USER_AGENT)
        page = context.new_page()

        for idx, artist_data in enumerate(
            artist_sources.items(), start=1
        ):  # 给列表每个歌手做一次getartist_detail，最后保存
            ...
            try:
                ...
                if detail_artist is None:
                    ...
                    detail_artist["artist_name"] in saved_artist_names
                ):  # 不对同一个歌手重复进行
                    print("no")
                else:
